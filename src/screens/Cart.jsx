...
import {useCart, useDispatchCart } from '../component/ContextFile'
import trash from "../screens/trash.svg";
...
export default function Cart() {
  ...
  return (
    ...
    <tbody>
     {data.map((food,index)=>(
         <tr style={{fontSize:"16px",fontWeight:"500"}}>
  ...
  <div><h1 className='fs-2'>Total Price: {totalPrice}/-</h1></div>
        <div>
          <button className='btn bg-primary mt-2 mx-5 ' onClick={handleCheckOut} style={{color:"#ffff"}} > Check Out </button>
        </div>
        </div>
       </div>
  )
